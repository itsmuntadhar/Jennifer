...
void setToZero()
{
    for (byte i = 0; i < 3; i++)
    {
        ...
        {
            EEPROM.write(0x10 + j + (i * sizeof(int16_t)), customOffsets[i] >> (sizeof(int16_t) * j));
        }
    }
}
...
void checkCustomOffsets()
{
    ...
    else
    {
        for (byte i = 0; i < 3; i++)
        {
            byte vals[sizeof(int16_t)];
            customOffsets[i] = 0;
            for (byte j = 0; j < sizeof(int16_t); j++)
            {
                customOffsets[i] |= (EEPROM.read(0x10 + j + (i * sizeof(int16_t))) << (sizeof(int16_t) * j));
            }
        }
    }
}
